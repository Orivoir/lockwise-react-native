...
const AccountEdit = ({route, navigation}) => {
  ...
  const onSubmit = () => {
    ...
    // is immuable
    const param = actionType === "create" ? fields.current: {
    ...
    .then(accountBack => {
      if(!accountBack) {
        if(actionType === "create") {
          ...
          console.warn('Oops, has ask update a account not exists from the storage');
        ...
        // synchronize store with the storage
        const ACTION_NAME = actionType === "update" ? "UPDATE_ACCOUNT" :"ADD_ACCOUNT";
    ...
    .catch(error => {
      console.error(`storage edit account for action ${actionType} has crash with: ${error.message}`);
