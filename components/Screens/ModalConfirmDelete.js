...
const ModalConfirmDelete = ({route,navigation}) => {
  ...
  const onDelete = () => {
    ...
    .then(accountRemoved => {
      ...
      if(!accountRemoved) {
        console.warn('Oops, account ask remove not exists into storage for: ', account);
      } else {
    ...
    .catch(error => {
      console.error(`apis local has fail remove account for: "${account.id}" with: ${error.message}`);
