...
import {SAFE_SSID} from './../constants';

const FabSynchronize = ({network, onSynchronize}) => {
  const [isAvailableServerSync, setIsAvailableServerSync] = React.useState(
    false,
  );
  ...
  const isAvailableNetwork =
    ...
    network.details.ssid === SAFE_SSID;
  ...
  if (isAvailableNetwork) {
    isAvailable()
      ...
      .catch(error => {
        console.error(
          `check status server sync has crash with: ${error.message}`,
        );
